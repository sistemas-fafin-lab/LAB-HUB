...
interface ColetaCardProps {
  ...
  cancelling?: boolean
  /** Abre a linha do tempo (detalhe) do agendamento. */
...
export function ColetaCard({
  ...
  const isConcluido = isCancelado || isRealizado
  ...
  return (
    ...
    >
      <div className="flex items-center gap-4">
        {/* Info principal — com opacidade reduzida quando concluído */}
        <div className={`flex items-center gap-4 flex-1 min-w-0 ${isConcluido ? 'opacity-60' : ''}`}>
        ...
        {clickable && (
          <div className={`shrink-0 ${isConcluido ? 'opacity-60' : ''}`}>
            <WIcon
              name="chevron-right"
